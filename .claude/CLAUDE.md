...
| Field | Value |
|---|---|
| Last completed slice | Slice 7 + post-slice UI fixes (see prior entries). Schema + fetcher change (2026-04-11): crypto symbol codes changed from full Binance pair to asset identifier (BTCUSDT→BTC, ETHUSDT→ETH); stablecoin type added with USDT global seed; crypto fetcher now derives Binance pair from code + primary_conversion_fiat at runtime (USD→USDT suffix mapping); fiat-leg validation added to createTransaction Server Action (trades require ≥1 fiat_currency or stablecoin leg); AssetRef.symbolType added; KNOWN_FIAT sets replaced with isFiatLike(ref) predicate. Migrations: 20260411000001, 20260411000002. Accounts page performance (2026-04-11): account switching now uses client state + window.history.replaceState (no server re-render per click); page load parallelized from 9 sequential queries to 4 round trips via Promise.all; latest_exchange_rates view added (DISTINCT ON symbol_id — no unbounded history transfer); two symbols queries merged into one .or() call; redundant profiles query removed (display_name now from membership join). Migration: 20260411000003. |
| Next slice | Slice 8 (TBD) |
| Known issues | `tefas-crawler` package does not exist on npm — tefas.ts uses direct HTTP to tefas.gov.tr instead. `COLLECTAPI_ENABLED=false` in .env.local — gold fetches skipped locally until Google Sheets is decommissioned. G/L is null for assets with no snapshot yet (new households before first snapshot). Display logic in `UI_PAGES.md` §4.2 includes a defensive case for both-non-fiat trades — this case is now blocked by Server Action validation but the display branch remains as a safe fallback. Accounts mobile polish (accordion expand/collapse behaviour) is explicitly deferred to a future slice. Rates symbol detail chart is pending a candlestick rewrite (using Recharts Bar range + ErrorBar pattern) — deferred to next session. |
